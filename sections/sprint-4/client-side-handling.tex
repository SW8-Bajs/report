...
When a hexadecimal is right bit shifted 4 bits every decimal in the hexadecimal is moved one decimal to the right.
\begin{center}
    0x123 >> 4 -> 0x012   
\end{center}
...
This means that because one decimal corresponds to four bits, the message has to be right bit shifted by 8 bits to move the timestamp to the back of the hexadecimal.
\begin{center}
...
\end{center}
Because the player id is represented by two decimals it can be decoded from the message by typecasting the message to the \textit{byte} datatype in C\#.
This is done for each part of the message where the message is right bit shifted by the combined size of the previously read parts and then typecasted to a datatype that has the same size as the part of the message that is being read.
