...
When the connection is acknowledged it transitions to \uppLoc{Connected} and then sends configurations to the \uppProc{Client} and transitions back to the lobby.
...
InGame the \uppProc{Host} can either send tag position through UDP, send goal postion and goal scored through TCP or end the game through TCP and transition back to \uppLoc{Lobby}.
\begin{figure}[h]
    \centering
...
The \uppProc{Client} starts in the \uppLoc{Initial} it then tries to connect and transitions to \uppLoc{Connecting}.
While in \uppLoc{Connecting} it waits for a acknowledgement from the \uppProc{Host}.
If it does not recieve a acknowledgement before the timeout limit it outputs a \uppOut{TCP_timeout} and transitions back to \uppLoc{Initial}.
Otherwise if the \uppProc{Client} recieves a acknowledgement from the \uppProc{Host} it transitions to \uppLoc{Setup} and waits for the \uppProc{Host} to send configurations.
When the \uppProc{Client} recieves the configurations it transitions to \uppLoc{Lobby} and then waits for the game to start.
When the \uppProc{Host} starts the game, all \uppProc{Clients} that are in \uppLoc{Lobby} transition to \uppLoc{InGame}.
From there the \uppProc{Client} waits for the \uppProc{Host} to send messages.
It can recieve a TCP message or a UDP message which makes it transition to InGame again or it can recieve a message that the game has ended which makes it transition to \uppLoc{Initial}.
\begin{figure}[h]
    \centering
