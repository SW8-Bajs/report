...
This lead to the previous \uppaal model specified in \autoref{sec:sprint3-uppaal} being outdated and that it had to be updated to fit the new requirements.
\\
...
\begin{uppaalcode}[caption={System declarations}, label={lst:uppaal4:systemdecl}]
...
Each place in the array corresponds to a buffer related to a client process so that \uppProc(Client(0)) will read from the 0th index of the array, \uppProc(Client(1)) will read from the second and so on.
...
\begin{uppaalcode}[caption={Global declarations}, label={lst:uppaal4:globaldecl}]
...
Everytime a \uppTemp{Client} has connected the Host receives a \uppIn{TCP_client_connected} synchronisation and \uppVar{connected_client} is incremented.
When all of the \uppTemp{Clients} have connected the \uppTemp{Host} moves on the next section.
In this section the \uppTemp{Host} uses the \uppOut{TCP_send} synchronisation to notify each \uppTemp{Client} that the game is starting through TCP.
Finally the \uppTemp{Host} is in-game and continuously sends messages to the \uppTemp{Clients}.
This is done through either \uppOut{TCP_send} synchronisation or \uppOut{UDP_send} synchronisation which are the TCP and UDP channels respectively.
\begin{figure}[h]
    \centering
...
\end{figure}
\begin{uppaalcode}[caption={local Host declarations}, label={lst:sprint-4-host-code}]
...
The \uppTemp{Client} has four primary locations, as seen on \autoref{fig:sprint4-uppaal-client}, which correspond to the states that the actual implementation can be in.
These four can be split into two groups\\
The \uppLoc{Initial} location which is the state of the client before it attempts to connect to a host, \uppLoc{Connecting} for when it is connecting to a hosted game instance, and the \uppLoc{Lobby} connection for when the connection has been established, but the game has not yet started.
...
The \uppTemp{TCP_Host} and \uppTemp{TCP_Client} were designed such that there will be a process of each for each client process that exists.
...
As seen on \autoref{fig:sprint4-uppaal-tcp-client} and \autoref{fig:sprint4-uppaal-tcp-host}, the TCP templates will stay in a \uppLoc{Start} location until they receive an \uppSync{input syncronization} from either a \uppTemp{Host} or \uppTemp{Client}.
...
Once the handshake has been completed, the \uppTemp{TCP_Host} will send a message to the \uppTemp{Host} to inform that a new player has been connected via the \uppIn{TCP_client_connected} synchronization, and then enter an \uppLoc{Idle} location to indicate that it is ready to send messages.
Likewise, in the \uppTemp{TCP_Client}, after the handshake is completed it will inform the \uppTemp{Client} that a connection has been successfully established, and enter an idle state where it is ready to receive messages.
...
When the \uppTemp{UDP_host} has started it waits for a \uppIn{UDP_send} synchronisation from the host and then notifies the \uppTemp{Clients} through the \uppOut{read_UDP_buffer} synchronisation that they should read the global buffer.
\begin{figure}[h]
    \centering
...
\uppTemp{Client(1)} Then also connects through threeway handshake without any timeouts.
Finally the after both of the clients are in \uppLoc{Lobby} the \uppTemp{Host} transitions to \uppLoc{InGame}.
\begin{figure}[h]
    \centering
...
\uppTemp{Client(1)} connects without any issues, but \uppTemp{Client(0)} times out once while connecing.
This can be seen on the trace of \uppTemp{TCP_client(0)}. When it sends the \uppOut{tcp_sync[0]} it timesout because it doesn't get are response from the \uppTemp{TCP_host(1)}.
...
\begin{figure}[h]
    ...
    \caption{Example of a trace where the \uppTemp{TCP_host} times out one while connecting.}
