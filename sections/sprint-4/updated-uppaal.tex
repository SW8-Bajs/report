...
We created an abstracted model to get an overview of the system and a more extensive model that also models the TCP communication between the host and the client.
The abstracted model will be described first in order to get a general overview of the flow of the system.

\subsection{Abstracted \uppaal model}
There are two templates in the abstracted model which are \uppTemp{Host} and \uppTemp{Client}.

\subsection*{Host}
The abstracted host can be seen on \autoref{fig:sprint4-uppaal-simplified-host}. 
...
\subsection*{Client}
The abstracted client can be seen on  \autoref{fig:sprint4-uppaal-simplified-client}.
...
\uppVar{connected_clients} which it uses to keep track of how many clients have connected, \uppVar{local_TCP_buffer} which is where the content of the global TCP buffer is copied into and \uppVar{i} which it uses to keep track of how many messages it has sent. 
...
It then transitions to an in-between location and then either transitions back to \uppLoc{lobby} or to \uppLoc{InGame} depending on what message is received.
...
You can then see the trace of each process which is the line that goes from the process to the bottom of the diagram.
The boxes on the trace indicate which \uppLoc{Location} the process is in at any given time.
