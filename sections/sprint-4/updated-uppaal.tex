...
In this new version of the model, the inclusion of \uppTemp{TCP_clients}, \uppTemp{TCP_hosts}, and a \uppTemp{UDP_host} allows us to model the behavior of the network aspect of the system.
...
The channels are used to synchronize between the different processes.
Where they are used will be explained when we describe each template.
...
Everytime a \uppTemp{Client} has connected the Host receives a \uppIn{TCP_client_connected} synchronization and \uppVar{connected_client} is incremented.
...
This is done through either \uppOut{TCP_send} synchronization or \uppOut{UDP_send} synchronization which are the TCP and UDP channels respectively.
\begin{figure}[h]
    \centering
...
The \uppTemp{Client} has four named locations, as seen on \autoref{fig:sprint4-uppaal-client}, which corresponds to the states that the actual implementation can be in.
\uppLoc{Initial} is the initial location of the \uppTemp{Client}.
It then uses the \uppOut{TCP_connect[id]} to make the \uppTemp{TCP_client(id)} start connecting through TCP and transition to the \uppLoc{Connecting} location.
The \uppTemp{Client(id)} recieves \uppIn{TCP_connected[id]} synchronization when the \uppTemp{TCP_client(i)} has connected and it then transitions to the \uppLoc{Lobby} location.
Everytime it recieves a \uppIn{read_TCP_buffer[id]} it means that the \uppTemp{Host} sent a message.
It then transitions to an in-between location and then either transition back to \uppLoc{lobby} or to \uppLoc{InGame} depending on what message it received.
The in-between location is a committed location.
This is done so that the \uppTemp{Client(id)} has to transition from it immediately instead of waiting.
...
As seen on \autoref{fig:sprint4-uppaal-tcp-client} and \autoref{fig:sprint4-uppaal-tcp-host}, the TCP templates will stay in a \uppLoc{Start} location until \uppTemp{TCP_client} receive an \uppIn{TCP_connect[id]} synchronization from a \uppTemp{Client}.
...
This can be seen on the trace of \uppTemp{TCP_client(0)}. When it sends the \uppOut{tcp_sync[0]} it times out because it does not get a response from the \uppTemp{TCP_host(1)}.
