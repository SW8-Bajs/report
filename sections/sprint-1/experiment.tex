...
The settings can be changed at a later point to get try and get the update rate to a level where the latency for the users is acceptable.\\
...
The blackboard would be placed in one of the fields defined in \autoref{fig:experiment-setup}, and the tags would be placed in certain positions on the board and record the accuracy with the position was reported.
...
Once one field had been tested, the blackboard was moved to the next field and repeated the setup.
...
One thing that could have affected the tags is that a blackboard was used as a measure for positions. 
