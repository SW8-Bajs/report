...
As it is needed to send data over the network for our game, it is required to define the format it is sent as.
...
These formats can be seen on \autoref{Tab:anchor-format}, \autoref{Tab:player-format} and \autoref{Tab:player-format}.
These are almost identical, as the same information is required.
The only difference between them is the type, and the context of the IDs.
The type is at the end of the hexadecimal, so that it is easy to bitshift, and then switch on which type it is.
After the type the timestamp is placed, which is used to detect outdated data.
\\\\
It was decided to use 2 and 4 hexadecimal, so that it is in byte format and hereby straightforward to work with in \texttt{C\#}.
Given the highest number that can be in two bytes are $2^{16} - 1$, the maximum number for the \texttt{x} and \texttt{y} positions are 65.535. 
\begin{table}[h!]
    \centering
    ...
    4 hexadecimal                                                 & 4 hexadecimal                                                 & 2 hexadecimal                                          & 2 hexadecimal                                                 & 2 hexadecimal                                        \\ \hline
    \end{tabular}
...
\begin{table}[h!]
    ...
    \begin{tabular}{|l|l|l|l|l|}
    \hline
    \begin{tabular}[c]{@{}l@{}}POSY (y)\\ (0-65.535)\end{tabular} & \begin{tabular}[c]{@{}l@{}}POSX (x)\\ (0-65.535)\end{tabular} & \begin{tabular}[c]{@{}l@{}}ID \\ (0-4)\\ 0 = Ball\\ 1-4 = Player\end{tabular} & \begin{tabular}[c]{@{}l@{}}TIMESTAMP (s)\\ (0-255)\end{tabular} & \begin{tabular}[c]{@{}l@{}}TYPE (t)\\ (1)\end{tabular} \\ \hline
    4 hexadecimal                                                 & 4 hexadecimal                                                 & 2 hexadecimal                                                                 & 2 hexadecimal                                                 & 2 hexadecimal                                        \\ \hline
    \end{tabular}
