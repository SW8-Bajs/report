...
For \autoref{Tab:goal-position-format} the \texttt{x} and \texttt{y} position is the center of the goal, where the size of the goal is set in unity.
The IDs for \autoref{Tab:anchor-format} is for each anchor.
For \autoref{Tab:player-format} the ID 0 is the ball, where 1-4 is a player and for \autoref{Tab:goal-position-format} the ID identifies what team it is..
...
\\\\
It was decided to use a size of respectively 16 and 8 bits so that it is straightforward to work with in the \texttt{C\#} language, because it is the same size as some variables in \texttt{C\#} and hereby easy to type cast.
Given the highest number that can be in two bytes are $2^{16} - 1$, the maximum value for the \texttt{x} and \texttt{y} positions are 65.535.
