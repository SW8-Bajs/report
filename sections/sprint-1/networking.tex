...
It aims to be a high-performance library to be used in distributed or concurrent applications that is reliable.
According to the getting started guide provided by ZeroMQ, there are certain issues that tend to arise when developers attempt to network through their own socket solution\cite{zeromqguide}.
These are:
\begin{itemize}
    ...
    \item How to represent data such that it can be read between different architectures? How much of this should be the messaging systems job?
...
However, for the purposes of this project, not all issues are relevant.
...
A message can be lost since during the playing of the game. 
Messages should be delivered to all players as to ensure they all have the same information.
...
\begin{table}[tbp]
    ...
    Requirements    & Familiarity with the solution                               & Familiarity with the knowledge required to implement a usable solution            & Needs a familiarity with a mix of pre-existing and custom solutions                     \\ \hline
